...
import { normalizeMealPlannerStore } from '@/lib/meal-planner';

const SUBJECT_KEY = 'meal-planner';
...
export async function GET() {
  try {
    ...
    const store = normalizeMealPlannerStore(row?.checklist ?? null);
    ...
    return handleApiError(error, 'api/meal-planner:GET');
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const store = normalizeMealPlannerStore(body?.store);
    ...
    return handleApiError(error, 'api/meal-planner:POST');
  }
}
