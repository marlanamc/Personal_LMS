...
export async function GET() {
  try {
    ...
    const store = normalizeMediaHubStore(row?.checklist ?? null);
    ...
    return handleApiError(error, 'api/media-hub:GET');
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const store = normalizeMediaHubStore(body?.store);
    ...
    return handleApiError(error, 'api/media-hub:POST');
  }
}
