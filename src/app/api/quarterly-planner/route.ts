...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      store: normalizeQuarterlyPlannerStore(row?.checklist ?? null),
      updatedAt: row?.updatedAt ?? null,
    });
  } catch (error) {
    return handleApiError(error, 'api/quarterly-planner:GET');
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return handleApiError(error, 'api/quarterly-planner:POST');
  }
}
