                    ...
                    <div className="flex justify-center flex-col items-center mb-8">
                        ...
                        {activeSequence && activeSequenceIndex !== null && (
                            <div className="mt-4 flex flex-col items-center">
                                {/* Transition overlay when moving to next block */}
                                {sequenceTransitionState === 'transitioning' && nextBlockLabel && (
                                    <div className="mb-4 px-6 py-4 rounded-2xl bg-gradient-to-r from-accent-teal/20 to-accent-sakura/20 border border-accent-teal/30 text-center animate-pulse">
                                        <p className="text-sm font-bold text-text">Starting next block...</p>
                                        <p className="text-lg font-display font-bold text-accent-teal mt-1">{nextBlockLabel}</p>
                                        <button
                                            onClick={() => {
                                                triggerHaptic(20);
                                                skipTransition();
                                            }}
                                            className="mt-3 px-4 py-2 rounded-full bg-accent-teal text-bg-base text-xs font-bold hover:brightness-110 transition-all"
                                        >
                                            Start Now
                                        </button>
                                    </div>
                                ...
                                {/* Paused state */}
                                {sequenceTransitionState === 'paused' && (
                                    <div className="mb-4 px-6 py-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-center">
                                        <div className="flex items-center justify-center gap-2 mb-2">
                                            <Coffee className="w-5 h-5 text-amber-500" />
                                            <p className="text-sm font-bold text-text">Taking a break</p>
                                        </div>
                                        <p className="text-xs text-text-muted mb-3">Your sequence is paused. Resume when ready.</p>
                                        <button
                                            onClick={() => {
                                                triggerHaptic(20);
                                                resumeSequence();
                                                setSessionNotice('Sequence resumed.');
                                            }}
                                            className="px-5 py-2.5 rounded-full bg-accent-teal text-bg-base text-sm font-bold hover:brightness-110 transition-all"
                                        >
                                            Resume Sequence
                                        </button>
                                    </div>
                                ...
                                <div className="flex flex-wrap gap-2 justify-center">
                                     {/* Take a Break button - only show when timer is active */}
                                     {isActive && sequenceTransitionState === 'idle' && (
                                         <button
                                             onClick={() => {
                                                 triggerHaptic(20);
                                                 pauseSequence();
                                                 setSessionNotice('Sequence paused. Take a break!');
                                             }}
                                             className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-500/10 hover:bg-amber-500/20 text-xs font-semibold text-amber-600 dark:text-amber-400 transition-colors border border-amber-500/30"
                                         >
                                             <Coffee className="w-3.5 h-3.5" />
                                             Take a Break
                                         </button>
                                     )}
                                     {activeSequenceIndex < activeSequence.length - 1 && sequenceTransitionState !== 'transitioning' && (
                                         <button
                                             onClick={() => {
                                                 triggerHaptic(20);
