...
function useTweaks(defaults) {
  ...
  const setTweak = React.useCallback((keyOrEdits, val) => {
    // Accept either (key, val) or ({key1: val1, key2: val2})
...
function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  if (!open) return null;
  // Portal to body so we escape the scaled #root transform (which would
  // otherwise break position:fixed and shrink the panel).
